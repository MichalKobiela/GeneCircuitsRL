...
function solve_prob(parameters, design, unc_params ,seed = nothing)
...
function maxgrowth(parameters, unc_params ,seed = nothing)
    _, growth_max = solve_prob(parameters, [0,0], unc_params ,seed)
    return growth_max
end
...
# --- Settings ---
numg0_values = exp.(collect(0:.01:10))   # grid of inductions
N_unc = 1000                              # number of uncertainty samples
thresholds = 0.3:0.05:0.9                 # growth thresholds to consider
Random.seed!(0)

# Generate random uncertainty samples
unc_samples = [rand(3) for _ in 1:N_unc]

# Prepare storage for dataset
dataset = DataFrame(unc1=Float64[], unc2=Float64[], unc3=Float64[], threshold=Float64[], optimal_numg0=Float64[])

# --- Generate dataset ---
@time for unc in tqdm(unc_samples)
    # Compute max growth for normalization (almost zero induction)
    _, max_growth = solve_prob(parameters, [0], unc)
    
    # Precompute expression and growth over the grid
    final_p_values = Float64[]
    final_growth = Float64[]
    
    for numg0_val in numg0_values
        final_p, gr = solve_prob(parameters, [numg0_val], unc)
        push!(final_p_values, final_p[16])
        push!(final_growth, gr / max_growth)  # normalize growth
    end
    
    # For each threshold, find optimal numg0
    for th in thresholds
        idx_allowed = findall(x -> x >= th, final_growth)
        if !isempty(idx_allowed)
            allowed_p = final_p_values[idx_allowed]
            i_max = idx_allowed[argmax(allowed_p)]
            optimal_numg0 = numg0_values[i_max]
        else
            optimal_numg0 = NaN
        end
        
        push!(dataset, (unc[1], unc[2], unc[3], th, optimal_numg0))
    end
end # time



println("Dataset ready! Number of samples: ", nrow(dataset))
first(dataset, 10)  # preview first 10 rows
CSV.write("training_dataset.csv", dataset)