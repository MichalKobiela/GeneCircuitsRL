...
const LEAKAGE_MIN, LEAKAGE_MAX = 0.05, 0.15
const GAMMA_X_MIN, GAMMA_X_MAX = 0.8, 1.1
...
# Loss based on frequency deviation
...
# Frequency from signal
function freq(signal)
    acf = autocorrelation(signal)
    return second_peak_from_acf_loc(acf)
end

# Updated unnormalization with new parameter split:
# uncertain = H, γX, γm, ϵ
# design = kX, km, K
function unnorm_params(normalized_params)
    ...
    ϵ  = denormalize_param(normalized_params[4], LEAKAGE_MIN, LEAKAGE_MAX)
    kX = denormalize_param(normalized_params[5], KX_MIN, KX_MAX)
    km = denormalize_param(normalized_params[6], KM_MIN, KM_MAX)
    K  = denormalize_param(normalized_params[7], K_MIN, K_MAX)
...
Random.seed!(123)
for i in 1:500
    params_unc = rand(4) * 2 .- 1   # H, γX, γm, ϵ
    params_design = rand(3) * 2 .- 1  # kX, km, K
...
m_obs, std_obs = mean(maxs), std(maxs)
m_freq, std_freq = mean(freqs), std(freqs)
m_reward, std_reward = mean(rewards), std(rewards)
...
# Example normalized params
params_unc = rand(4) * 2 .- 1
params_design = rand(3) * 2 .- 1
params_arr = vcat(params_unc, params_design)
...
params = zeros(7)  # now 4 uncertain + 3 design
...
design_paper_normalized = [
    normalize_param(design_paper[1], KX_MIN, KX_MAX),
    normalize_param(design_paper[2], KM_MIN, KM_MAX),
    normalize_param(design_paper[3], K_MIN, K_MAX)
]
unc_paper_normalized = [
    normalize_param(unc_paper[1], H_MIN, H_MAX),
    normalize_param(unc_paper[2], GAMMA_X_MIN, GAMMA_X_MAX),
    normalize_param(unc_paper[3], GM_MIN, GM_MAX),
    normalize_param(unc_paper[4], LEAKAGE_MIN, LEAKAGE_MAX)
]
...
# Random uncertainty + paper design
params_unc = rand(4) * 2 .- 1
