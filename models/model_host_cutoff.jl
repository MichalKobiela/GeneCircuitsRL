...
final_growth = final_growth ./ maximum(final_growth)

plot(final_p_values, xlabel = "induction", ylabel = "expression")
plot!(legend=nothing)
plot(final_growth,  xlabel = "induction", ylabel = "growth")
...
# barrier term
using NaNMath
barrier = NaNMath.log.(-h)      # log-barrier function
plot(barrier, xlabel = "induction", ylabel ="barrier")
# penalized objective
plot(final_p_values, xlabel = "induction", label = "no barrier")
α= 0.1
p_barrier = final_p_values .+ α .* barrier
...
function compute_obj_with_growth(design,
    ...
    p_raw = p_final + α * raw_barrier

    p_clip = isnan(p_raw) ? smooth_tanh(h; k=k) : max(p_raw, 0.0)
    return p_clip
end

obj_val = compute_obj_with_growth([10], unc)

obj_vals = Float64[]
for numg0_val in numg0_values
    val = compute_obj_with_growth([numg0_val], unc)
    push!(obj_vals, val)
end
plot(obj_vals)

function reward(numg0)
    return compute_obj_with_growth([exp(numg0*10)], unc)
end

function experession_and_growth(unc,numg0_val)
    y_end, growth = solve_prob(parameters, [numg0_val], unc)
